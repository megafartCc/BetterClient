...
local AUTOFARM_POOF_DAMAGE_RANGE = 480
    ...
    for i = 1, #meepo_entries do
        ...
        if meepo and key and Entity.IsAlive(meepo) and not is_combo_cast_in_progress(meepo) then
            ...
            if valid and assigned and assigned.has_creeps ~= true then
                ...
                if assigned_ready_at > now_time then
                    valid = false
                end
            end
            if not valid then
                        ...
                        if stack_active then
                            ...
                            if target then
                                ...
                                if use_poof_damage and mana_pct > mana_reserve_pct and #creeps >= AUTOFARM_POOF_DAMAGE_MIN_CREEPS then
                                    local poof = safe_call(NPC.GetAbility, meepo, C.ABILITY_POOF)
                                    local dist = vec_dist_2d(meepo_pos or assigned.world, safe_call(Entity.GetAbsOrigin, target) or assigned.world)
                                    if poof and dist <= AUTOFARM_POOF_DAMAGE_RANGE and safe_call(Ability.IsReady, poof) == true then
        ...
        if camp and camp.id then
            ...
            if center then
                ...
                if visible and screen then
                    local status_name = (state and state.status) or (status_state and status_state.state) or "ready"
